...
export default function CryptoPage() {
  const { rows, totalValueUsd, isLoadingPrices, priceError } = useCryptoPortfolio()
      ...
      <main className="max-w-md mx-auto px-5 py-6 space-y-6">
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-3">
            {rows.map(({ holding, stats, quote, currentValueUsd, returnRate }) => {
              ...
              return (
                <Link
                  key={symbol}
                  href={`/assets/crypto/${encodeURIComponent(symbol)}`}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        <p className="text-[15px] font-black text-slate-800">{holding.unit ?? symbol.split("-")[0]}</p>
