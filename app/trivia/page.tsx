...
import { Lightbulb, Trophy, AlertCircle, Sparkles, Lock, CheckCircle, XCircle, Bone, Zap } from "lucide-react"
...
//
// XP 연동 변경 사항:
// - 정답 시 useUserProfile().addXp("quiz_correct", 50, "퀴즈 정답") 호출
// - 레벨업 여부를 result 화면에서 표시
// - 상단에 현재 XP/레벨 뱃지 표시 (sm 사이즈)
// =============================================================================
...
export default function TriviaPage() {
  ...
  const { profile, currentLevel, nextLevel, levelProgress, addXp } = useUserProfile()
  ...
  useEffect(() => {
    const loaded = JSON.parse(localStorage.getItem("solvedTrivia") || "{}")
    setSolvedStatus(loaded)
  }, [])
  ...
  const handleSubmit = () => {
    ...
    const newStatus = {
      ...solvedStatus,
      [`week${currentWeekIdx + 1}_${selectedCat}`]: { solved: true, correct },
    }
    setSolvedStatus(newStatus)
    localStorage.setItem("solvedTrivia", JSON.stringify(newStatus))

    // ✨ XP 지급 — 정답일 때만 addXp 호출
    if (correct) {
      const result = addXp("quiz_correct", XP_PER_CORRECT, `${quizData[selectedCat].title} 퀴즈 정답`)
  ...
  return (
    <div className="min-h-dvh bg-[url('https://i.ifh.cc/GGjN8Q.webp')] bg-cover bg-center bg-no-repeat bg-fixed pb-20 relative">
      ...
      <DetailHeader
        ...
        onBack={() => { if (step === "select") router.push("/") else setStep("select") }}
      />
        ...
        <div className="bg-white/20 backdrop-blur-xl rounded-3xl p-6 border border-white/30 shadow-2xl relative">
          ...
          {step === "select" && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <p className="text-[11px] font-bold text-white/60 mb-4">✨ 정답 1개당 {XP_PER_CORRECT} XP 획득</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                {(Object.keys(quizData) as Category[]).map((key, idx) => {
                  ...
                  return (
                    <button key={key} onClick={() => !isLocked && !status?.solved && handleSelect(key)}
            ...
            <div className="animate-in fade-in zoom-in-95 duration-500 text-white">
              <div className={`rounded-2xl bg-gradient-to-br ${quizData[selectedCat].color} p-5 mb-6 border border-white/20 shadow-inner`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1 bg-white/10 px-2 py-1 rounded-full">
                    ...
                    <span className="text-[10px] font-black text-amber-200">+{XP_PER_CORRECT} XP</span>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col items-center">
                  ...
                  {showHint && (
                    <div className="mt-4 p-4 bg-amber-100/90 backdrop-blur-md border border-amber-300 rounded-xl text-xs font-bold text-amber-900 animate-in fade-in w-full text-center shadow-md">
          ...
          {step === "result" && (
            <div className="flex flex-col items-center py-10 text-white text-center animate-in zoom-in-95">
              {isCorrect ? (
                ...
                <AlertCircle className="w-20 h-20 text-rose-400 mb-6 drop-shadow-[0_0_15px_rgba(251,113,133,0.5)]" />
              )}
              ...
              {isCorrect && (
                <div className="flex items-center gap-2 mb-4">
                  <div className="flex items-center gap-1.5 bg-amber-400/20 border border-amber-400/30 px-4 py-2 rounded-full backdrop-blur-sm">
                    <Zap className="w-4 h-4 text-amber-300" />
                    <span className="text-[13px] font-black text-amber-200">+{XP_PER_CORRECT} XP 획득!</span>
                  </div>
                  {leveledUp && (
                    <div className="flex items-center gap-1.5 bg-emerald-400/20 border border-emerald-400/30 px-4 py-2 rounded-full backdrop-blur-sm animate-in zoom-in">
              ...
              <p className="mb-10 text-sm font-medium opacity-90 leading-relaxed">
                {isCorrect
                  ? `경험치가 쌓이고 있어요. 마이페이지에서 레벨을 확인해 보세요!`
                  : "더 공부해서 다음 주에 다시 만나요!"}
              </p>
